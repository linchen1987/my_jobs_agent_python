...
需要**全部满足**以下判断标准：
1. 是招聘信息
2. 必须为长期的全职或兼职工作，不能是一次性项目
3. 是开发类工作，不是非开发类（比如产品运营）
...
def analyze_job_posting(
    title: str, content: str, categories: List[Dict]
) -> Dict[str, Any]:
    """
    分析招聘信息的工具函数
    ...
    return {
        "prompt": prompt,
        "input_data": {"title": title, "content": content, "categories": categories},
    }
    ...
    if tags:
        for tag in tags:
            if isinstance(tag, dict) and "category" in tag and "values" in tag:
                categories.append(
                    {"category": tag["category"], "values": tag["values"]}
                )
    return categories
...
def process_job_data(job_data: Dict) -> Dict[str, Any]:
    ...
    result["original_data"] = {
        "extra": job_data.get("extra"),
        "list_item": job_data.get("list_item"),
    }
...
def example_usage():
    ...
    example_data = {
        ...
        "tags": [
            {"category": "招聘类型", "values": ["外包零活"]},
            {"category": "职业", "values": ["开发"]},
            {"category": "工作方式", "values": ["线上兼职", "远程工作"]},
        ],
    }
...
def test_with_real_data():
    ...
    real_data = {
        "title": "需要一名thinkphp开发",
        "content": "需求\n\n维护已有CRM 系统。\n\n技术栈\n\nthinkphp6，可升级为 thinkphp8vue2、element-ui",
        "meta_info": {"reads": 259, "comments": 20},
        "tags": [
            {"category": "招聘类型", "values": ["外包零活"]},
            {"category": "职业", "values": ["开发"]},
            {"category": "工作方式", "values": ["线上兼职", "远程工作"]},
